...
# ==========================================
print("\n🔍 各個變數對房價的重要性 (Feature Importances)：")
print(f"{'特徵名稱 (Feature)':<20} | {'重要性分數 (Importance)':<25}")
print("-" * 50)

# 將特徵名稱與重要性分數配對並排序 (改用 model.feature_importances_)
importance = sorted(zip(feature_names, model.feature_importances_), key=lambda x: x[1], reverse=True)
for name, score in importance:
    print(f"{name:<20} | {score * 100:>6.2f}%")
